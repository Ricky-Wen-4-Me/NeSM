...
def load_segment(path):
    ...
    for file_name in file_list:
        ...
        #print(tf.reduce_any(tf.greater(img_array, 0.9))) # 255.
    ...
    img_tf_sqz = np.squeeze(img_tf_array)

    tensorData = tf.convert_to_tensor(img_tf_sqz/255., dtype=tf.float32)
    
    #print(tensorData.shape)
    #masked_values = tf.boolean_mask(tensorData, tensorData > 0.)
    #first_greater_value = tf.reduce_min(masked_values)

    #print(first_greater_value) # 0.99990004
    return tensorData
