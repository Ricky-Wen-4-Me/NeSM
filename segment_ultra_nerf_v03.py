#m3m6 & PoNe (m3m6: helper activate modulation)
import os
...
from segment_nerf_helper_v04 import *  #1 lyr, ReLU 
#from segment_nerf_helper import *     #2 lyrs, LeakReLU
from load_us import load_us_data, load_segment
...
def render_method_convolutional_ultrasound(raw, z_vals, segment, args):
    def raw2attenualtion(raw, dists):
        return tf.exp(-raw * dists)
    def raw2segmentation(raw, dists):
        return raw * dists
    ...
    intensity_map = b + r
    if segment:
        #segmentation_map = tf.math.abs(raw[..., 5])
       
        positive_occupied_value = raw[..., 5]

        positive_occupied = tf.multiply(tf.ones_like(positive_occupied_value), positive_occupied_value)
        
        negative_occupied = tf.subtract(tf.ones_like(positive_occupied), positive_occupied)
        negative_occupied = tf.math.abs(negative_occupied)

        p_seg = tf.multiply(intensity_map, positive_occupied)
        n_seg = tf.multiply(intensity_map, negative_occupied)
        
        annotated = tf.math.divide_no_nan(positive_occupied, 2.0)
        positive_segmentation = tf.math.add(p_seg, annotated)

        final_intensity_map = p_seg + n_seg
    ...
    ret = {'intensity_map': intensity_map,
           ...
           'final_intensity_map': final_intensity_map,
           'positive_occupied': positive_occupied,
...
def render_rays_us(ray_batch,
    ...
    def raw2outputs(raw, z_vals, rays_d, segment):
        """Transforms model's predictions to semantically meaningful values.
        """
        ret = render_method_convolutional_ultrasound(raw, z_vals, segment, args)
        return ret
...
def batchify_rays(rays_flat, c2w=None, chunk=32 * 256, **kwargs):
    ...
    for i in range(0, rays_flat.shape[0], chunk):
        ret = render_rays_us(rays_flat[i:i + chunk], segment=True, **kwargs)
        for k in ret:
            if k not in all_ret:
                all_ret[k] = []
...
def render_us(H, W, sw, sh,
    ...
    # Create rainstensity_map_reversey batchborder_convolution
...
def create_nerf(args):
    ...
    model = init_nerf_model(
        ...
        input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs, mode=1, modulation=False, use_segment=True)
...
def train():
    ...
    if args.dataset_type == 'us':
        ...
        return
    s_tar = load_segment("./data/correct_thyroid/seg_images/")
    print("s_tar : ", s_tar.shape)
    masked_values = tf.boolean_mask(s_tar, s_tar > 0.)
    first_greater_value = tf.reduce_max(masked_values)
    print("s_tar_max_value : ", first_greater_value)
    ...
    print("N_samples : ", render_kwargs_train["N_samples"])
    # Create optimizer
    lrate = args.lrate
    ...
    nerf_loss_1, nerf_loss_2, nerf_loss_3 = 0, 0, 0
    best_loss = float('inf')
    best_iteration = 0
    best_miou, best_pa, best_dice_c = 0., 0., 0.
    ...
    for i in range(start, N_iters):
        ...
        try:
            ...
            target_seg = tf.transpose(s_tar[img_i])
        except:
            print("random choose image number: ", img_i)

        pose = poses[img_i, :3, :4]
        ssim_weight = args.ssim_lambda * args.ssim_lambda
        c_entropy_weight = args.ssim_lambda * (1. - args.ssim_lambda)
        l2_weight = 1. - (ssim_weight + c_entropy_weight)
        #c_entropy_weight = 1. 
        #cosine_sim_weight = 1.
        #pos_weight = 0.5

        f_weight = args.ssim_lambda
        s_weight = 1. - args.ssim_lambda - (args.ssim_lambda * args.ssim_lambda)
        l2_weight = args.ssim_lambda * args.ssim_lambda
        ...
        with tf.GradientTape() as tape:
            ...
            output_image = rendering_output['final_intensity_map']
            output_annotate = rendering_output['positive_segmentation']
            output_segment = rendering_output['positive_occupied']

            if args.loss == 'l2':
                img1_mapping = tf.clip_by_value(output_image, 0.0, 1.0)
                img2_mapping = tf.clip_by_value(target, 0.0, 1.0)

                l2_intensity_loss = img2mse(output_image, target)
                print("l2_intensity_loss : ", l2_intensity_loss)
                loss["l2"] = (1., l2_intensity_loss)
            elif args.loss == 'ssim':
                img1 = tf.expand_dims(tf.expand_dims(output_image, 0), -1)
                img2 = tf.expand_dims(tf.expand_dims(target, 0), -1)
                img3 = tf.round(tf.expand_dims(tf.expand_dims(output_segment, 0), -1))
                img4 = tf.round(tf.expand_dims(tf.expand_dims(target_seg, 0), -1))
                ssim_intensity_loss = 1. - tf.image.ssim_multiscale(img1, img2,
                                                                    max_val=1.0, filter_size=args.ssim_filter_size,
                                                                    filter_sigma=1.5, k1=0.01, k2=0.1)
                print("ssim_intensity_loss : ", ssim_intensity_loss)
                loss["ssim"] = (ssim_weight, ssim_intensity_loss)
                
                l2_intensity_loss = img2mse(img1, img2)
                print("l2_loss : ", l2_intensity_loss)
                loss["l2"] = (l2_weight, l2_intensity_loss)
                
                masked_img3 = tf.boolean_mask(img3, img3 > 0)
                masked_img4 = tf.boolean_mask(img4, img4 > 0)
                
                exist_03 = tf.reduce_max(masked_img3)
                exist_04 = tf.reduce_max(masked_img4)
                
                print("Biggest number in img3: ", exist_03.numpy(), "Biggest number in img4: ", exist_04.numpy())

                bce = tf.keras.losses.BinaryCrossentropy(from_logits=True)
                entropy_intensity_loss = bce(img4, img3)

                print("entropy_intensity_loss : ", entropy_intensity_loss)
                loss["c_entropy"] = (c_entropy_weight, entropy_intensity_loss)
                
                dice, dice_loss = dice_coefficient(img3, img4)
                m_iou = mean_iou(img3, img4)
                pa = pixel_accuary(img3, img4)

            elif args.loss == 'double_ssim':
                img1 = tf.expand_dims(tf.expand_dims(output_image, 0), -1)
                img2 = tf.expand_dims(tf.expand_dims(target, 0), -1)
                img3_anno = tf.expand_dims(tf.expand_dims(output_annotate, 0), -1)
                img3 = tf.round(tf.expand_dims(tf.expand_dims(output_segment, 0), -1))
                img4 = tf.round(tf.expand_dims(tf.expand_dims(target_seg, 0), -1))

                feature_ssim_intensity_loss = 1. - tf.image.ssim_multiscale(img1, img2,
                                                                    max_val=1.0, filter_size=args.ssim_filter_size,
                                                                    filter_sigma=1.5, k1=0.01, k2=0.1)

                print("feature_ssim_intensity_loss : ", feature_ssim_intensity_loss)
                loss["feature_ssim"] = (f_weight, feature_ssim_intensity_loss)
                ...
                img4_seg = tf.math.multiply(img2, img4)
                img4_occup = tf.math.divide_no_nan(img4, 2.0)
                img4_anno = tf.math.add(img4_seg, img4_occup)

                segmentation_ssim_intensity_loss = 1. - tf.image.ssim(img3_anno, img4_anno,
                                                                        max_val=1.0, filter_size=args.ssim_filter_size,
                                                                        filter_sigma=1.5, k1=0.01, k2=0.1)

                print("segmentation_ssim_intensity_loss : ", segmentation_ssim_intensity_loss)
                loss["segmentation_ssim_intensity_loss"] = (s_weight, segmentation_ssim_intensity_loss)
                
                l2_intensity_loss = img2mse(img3, img4)
                print("l2_intensity_loss : ", l2_intensity_loss)
                loss["l2_intensity_loss"] = (l2_weight, l2_intensity_loss)
            ...
            if m_iou > best_miou:
                ...
                f_ssim = 1. - tf.constant(feature_ssim_intensity_loss).numpy()
                s_ssim = 1. - tf.constant(segmentation_ssim_intensity_loss).numpy()
                l2 = tf.constant(l2_intensity_loss).numpy()
                print("(f, s)", f_ssim, ",", s_ssim, "L2", l2)
                    
                nerf_loss_1, nerf_loss_2, nerf_loss_3 = f_ssim, s_ssim, l2
                best_miou, best_pa, best_dice_c = m_iou, pa, dice
        
        print("total_loss: ", total_loss)
        #print('mIoU: {0}, pixel accuary: {1}, dice: {2}'.format(m_iou, pa, dice_loss))
        print("best iteraion: ", best_iteration, "f, s: ", nerf_loss_1, nerf_loss_2, "L2: ", nerf_loss_3,
                        ", mIoU: ", tf.constant(best_miou).numpy(), 
                        ", pixel accuary: ", tf.constant(best_pa).numpy(), ", dice_coeff: ", tf.constant(best_dice_c).numpy())
        ...
        if i % args.i_print == 0 or i < 10:
            ...
            if i % args.i_img == 0:
                ...
                target = tf.transpose(s_tar[img_i])
                pose = poses[img_i, :3, :4]
                ...
                output_image_test = rendering_output_test['final_intensity_map']
                output_annotate_test = rendering_output_test['positive_segmentation']
                output_segment_test = rendering_output_test['positive_occupied']
                ...
                if args.loss == 'l2':
                    ...
                    print("entropy_intensity_loss : ", entropy_intensity_loss)
                    loss_holdout["c_entropy"] = (c_entropy_weight, entropy_intensity_loss)
                    ...
                    img1 = tf.expand_dims(tf.expand_dims(output_image_test, 0), -1)
                    img2 = tf.expand_dims(tf.expand_dims(target, 0), -1)
                    img3_anno = tf.expand_dims(tf.expand_dims(output_annotate_test, 0), -1)
                    img3 = tf.round(tf.expand_dims(tf.expand_dims(output_segment_test, 0), -1))
                    img4 = tf.round(tf.expand_dims(tf.expand_dims(target_seg, 0), -1))
                    ...
                    print("feature_ssim_intensity_loss : ", feature_ssim_intensity_loss)
                    loss_holdout["feature_ssim"] = (f_weight, feature_ssim_intensity_loss)
                    ...
                    print("segmentation_ssim_intensity_loss : ", segmentation_ssim_intensity_loss)
                    loss["segmentation_ssim"] = (s_weight, segmentation_ssim_intensity_loss)
                ...
                imageio.imwrite(os.path.join(testimgdir,
                                             'seg{:06d}.png'.format(i)), to8b(tf.transpose(tf.round(output_segment_test))))
                #imageio.imwrite(os.path.join(testimgdir,'border{:06d}.png'.format(i)), to8b(tf.transpose(output_border_test)))
                ...
                with tf.contrib.summary.record_summaries_every_n_global_steps(args.i_img):
                    ...
                    tf.contrib.summary.image('b_mode/output/',
                                             tf.expand_dims(tf.expand_dims(to8b(tf.transpose(output_image_test)), 0),
                                                            -1))
                    tf.contrib.summary.image('b_mode/output/',
                                             tf.expand_dims(tf.expand_dims(to8b(tf.transpose(output_segment_test)), 0),
                                                            -1))
