...
def init_nerf_model(D=8, W=256, input_ch=3, input_ch_views=3, output_ch=6, skips=[4], use_viewdirs=False, mode=0, modulation=False, use_segment=False):
    ...
    if mode == 0: #pseudo color
        #path = 'segment_data/result_US_image/pseudo_color_prediction/'
        path = 'segment_data/thyroid/pseudo_color_prediction/'
    elif mode == 1: #added
        #path = 'segment_data/result_US_image/added_prediction/'
        path = 'segment_data/thyroid/added_prediction/'
    
    modulation_v_set = feature_vector_transformation(W, path)
    print("mod_v_set_shape: ", modulation_v_set)
    ...
    if use_viewdirs:
        ...
        #outputs = dense(W//2, act=None)(outputs)
        #outputs = tf.keras.activations.sigmoid(outputs)
        occupy = dense(1, act=tf.keras.layers.ReLU(max_value=1.0))(outputs)

        outputs = tf.concat([bottleneck, occupy], -1)
    else:
    ...
    if modulation:
        ...
        dense_6_w = model.get_layer('dense_6').get_weights()
    #print(dense_3_w.shape)
        #print("dense_3_w: ", dense_3_w, len(dense_3_w))
        for i in range(len(dense_6_w[0])):
            dense_3_w[0][i] = dense_3_w[0][i] + modulation_v_set[i]
            dense_6_w[0][i] = dense_6_w[0][i] + modulation_v_set[i]

        model.get_layer('dense_3').set_weights(dense_3_w)
        model.get_layer('dense_6').set_weights(dense_6_w)
