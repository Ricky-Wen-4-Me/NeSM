#output6 (helper activate "modulation" and only use "dense_3")
import os
...
from segment_nerf_helper_v04 import *
...
def create_nerf(args):
    ...
    model = init_nerf_model(
        ...
        input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs, mode=1, modulation=True, use_segment=True)
...
def config_parser():
    ...
    parser.add_argument("--ssim_lambda", type=float, default=0.25)
...
def train():
    ...
    if args.dataset_type == 'us':
        ...
        return
    s_tar = load_segment("./segment_data/thyroid/pseudo_color_prediction/")
    ...
    for i in range(start, N_iters):
        ...
        s_weight = 1. - args.ssim_lambda
        #l2_weight = 1 - (f_ssim_weight + s_ssim_weight)
        ...
        with tf.GradientTape() as tape:
            ...
            output_segment = rendering_output['segmentation_map']
            if args.loss == 'l2':
                ...
                print("entropy_intensity_loss : ", entropy_intensity_loss)
                loss["c_entropy"] = (test_weight, entropy_intensity_loss)

                dice, dice_loss = dice_coefficient(img3, img4)
                m_iou = mean_iou(img3, img4)
                pa = pixel_accuary(img3, img4)
            elif args.loss == 'cosine_similarity': #not work yet
                img1 = tf.expand_dims(tf.expand_dims(output_image, 0), -1)
                img2 = tf.expand_dims(tf.expand_dims(target, 0), -1)
                img3 = tf.round(tf.expand_dims(tf.expand_dims(output_segment, 0), -1))
                img4 = tf.round(tf.expand_dims(tf.expand_dims(target_seg, 0), -1))
                ...
                feature_consine_sim_loss = 1. - cs(img2, img1)
                segmentation_cosine_sim_loss = 1. - cs(img4, img3)
                
                bce = tf.keras.losses.BinaryCrossentropy()
                cosine_sim_loss = bce(tf.math.subtract(feature_consine_sim_loss, pos_weight), segmentation_cosine_sim_loss)

                print("cosine_similarity_loss : ", cosine_sim_loss)
                loss["cosine_sim"] = (cosine_sim_weight, cosine_sim_loss)

                dice, dice_loss = dice_coefficient(img3, img4)
                m_iou = mean_iou(img3, img4)
                pa = pixel_accuary(img3, img4)
            elif args.loss == 'ssim_dice':
                img1 = tf.expand_dims(tf.expand_dims(output_image, 0), -1)
                img2 = tf.expand_dims(tf.expand_dims(target, 0), -1)
                img3 = tf.round(tf.expand_dims(tf.expand_dims(output_segment, 0), -1))
                img4 = tf.round(tf.expand_dims(tf.expand_dims(target_seg, 0), -1))
                ...
                masked_img3 = tf.boolean_mask(img3, img3 > 0.5)
                masked_img4 = tf.boolean_mask(img4, img4 > 0)
                exist_03 = tf.reduce_max(masked_img3)
                exist_04 = tf.reduce_max(masked_img4)
                ...
                print("segmentation_dice_loss : ", dice_loss)
                loss["segmentation_dice_loss"] = (s_weight, segmentation_dice_loss)
        ...
        if i % args.i_print == 0 or i < 10:
            ...
            if i % args.i_img == 0:
                ...
                output_segment_test = rendering_output_test['segmentation_map']
                ...
                if args.loss == 'l2':
                    ...
                    loss_holdout["c_entropy"] = (test_weight, entropy_intensity_loss)

                    dice, dice_loss = dice_coefficient(img3, img4)
                    m_iou = mean_iou(img3, img4)
                    pa = pixel_accuary(img3, img4)
                elif args.loss == 'cosine_similarity':
                    img1 = tf.expand_dims(tf.expand_dims(output_image, 0), -1)
                    img2 = tf.expand_dims(tf.expand_dims(target, 0), -1)
                    img3 = tf.round(tf.expand_dims(tf.expand_dims(output_segment, 0), -1))
                    img4 = tf.round(tf.expand_dims(tf.expand_dims(target_seg, 0), -1))
                    ...
                    print("cosine_similarity_loss : ", cosine_sim_loss)
                    loss_holdout["cosine_sim"] = (cosine_sim_weight, cosine_sim_loss)
                    ...
                    #segmentation_ssim_intensity_loss = 1. - tf.image.ssim_multiscale(img3, img4, max_val=1.0, filter_size=args.ssim_filter_size, filter_sigma=1.5, k1=0.01, k2=0.1)
                    dice, dice_loss = dice_coefficient(img3, img4)

                    print("segmentation_dice_loss : ", dice_loss)
                    loss_holdout["segmentation_dice_loss"] = (s_weight, dice_loss)
